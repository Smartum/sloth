...
`,
		...
		"Having a blank or empty rule_group interval should render with default interval.": {
			k8sMeta: k8sprometheus.K8sMeta{
				Name:        "test-name",
				Namespace:   "test-ns",
				Labels:      map[string]string{"lk1": "lv1"},
			...
			slos: []k8sprometheus.StorageSLO{
				{
					SLO: prometheus.SLO{
						ID:                    "testa",
						SLIErrorRulesInterval: sliErrorRulesInterval,
						MetadataRulesInterval: metadataRulesInterval,
						AlertRulesInterval:    alertRulesInterval,
					},
					Rules: prometheus.SLORules{
						SLIErrorRecRules: []rulefmt.Rule{
							{
			...
			slos: []k8sprometheus.StorageSLO{
				{
					SLO: prometheus.SLO{
						...
						SLIErrorRulesInterval: sliErrorRulesInterval,
						MetadataRulesInterval: metadataRulesInterval,
						RuleGroupInterval:     ruleGroupInterval,
					},
					Rules: prometheus.SLORules{
						SLIErrorRecRules: []rulefmt.Rule{
							{
...
`,
		...
		"Having multiple SLO alert and recording rules should render correctly.": {
			k8sMeta: k8sprometheus.K8sMeta{
				Name:        "test-name",
				Namespace:   "test-ns",
				Labels:      map[string]string{"lk1": "lv1"},
...
`,
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			repo := managedprometheus.NewIOWriterYAMLRepo(&gotYAML, log.Noop)
...
func TestPrometheusOperatorCRDRepo(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Having an error while storing Prometheus operator rules should fail.": {
			...
			mock: func(m *k8sprometheusmock.PrometheusRulesEnsurer) {
				m.On("EnsureManagedPrometheusRule", mock.Anything, mock.Anything).Once().Return(fmt.Errorf("something"))
			},
			expErr: true,
		},
		...
		"Having multiple SLO alert and recording rules should ensure on Kubernetes correctly.": {
			...
			mock: func(m *k8sprometheusmock.PrometheusRulesEnsurer) {
				...
				m.On("EnsureManagedPrometheusRule", mock.Anything, exp).Once().Return(nil)
			},
		},
	}
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			repo := managedprometheus.NewCRDRepo(mpre, log.Noop)
			err := repo.StoreSLOs(context.TODO(), test.k8sMeta, test.slos)
