...
var (
	specTypeV1RegexKind       = regexp.MustCompile(`(?m)^kind: +['"]?ManagedPrometheusServiceLevel['"]? *$`)
...
func (y YAMLSpecLoader) LoadSpec(ctx context.Context, data []byte) (*k8sprometheus.SLOGroup, error) {
	...
	obj, _, err := y.decoder.Decode(data, nil, nil)
	if err != nil {
		return nil, fmt.Errorf("could not decode kubernetes object %w", err)
	}

	kslo, ok := obj.(*v1.ManagedPrometheusServiceLevel)
	if !ok {
...
func (c CRSpecLoader) LoadSpec(ctx context.Context, obj runtime.Object) (*k8sprometheus.SLOGroup, error) {
	return mapSpecToModel(ctx, c.windowPeriod, c.pluginsRepo, obj.(*v1.ManagedPrometheusServiceLevel))
}

func mapSpecToModel(ctx context.Context, defaultWindowPeriod time.Duration, pluginsRepo k8sprometheus.SLIPluginRepo, kspec *v1.ManagedPrometheusServiceLevel) (*k8sprometheus.SLOGroup, error) {
