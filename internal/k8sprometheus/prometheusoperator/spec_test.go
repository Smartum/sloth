...
`,
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			loader := prometheusoperator.NewYAMLSpecLoader(testMemPluginsRepo(test.plugins), 30*24*time.Hour)
...
`,
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			loader := prometheusoperator.NewYAMLSpecLoader(testMemPluginsRepo(map[string]prometheus.SLIPlugin{}), 30*24*time.Hour)
