...
func sanitizeManagedPrometheusRule(pr *managedprometheusv1.Rules) *managedprometheusv1.Rules {
	pr = pr.DeepCopy()
...
func TestKubernetesControllerManagedPromOperatorGenerate(t *testing.T) {
	// Tests config.
	config := k8scontroller.NewConfig(t)
	version, err := testutils.SlothVersion(context.TODO(), config.Binary)
	require.NoError(t, err)
	...
	tests := map[string]struct {
		...
		"Having SLOs as a CRD should generate Managed Prometheus operator CRD.": {
			sloPeriod: "30d",
			exec: func(ctx context.Context, t *testing.T, ns string, kubeClis *k8scontroller.KubeClients) {
				// Prepare our SLO on Kubernetes.
				SLOs := getBaseManagedPrometheusServiceLevel()
				_, err := kubeClis.Sloth.SlothV1().ManagedPrometheusServiceLevels(ns).Create(ctx, SLOs, metav1.CreateOptions{})
				require.NoError(t, err)
				...
				// Check.
				expRule := getBaseManagedPromOpRules(version)
				expRule.Namespace = ns

				gotRule, err := kubeClis.ManagedPrometheus.MonitoringV1().Rules(ns).Get(ctx, expRule.Name, metav1.GetOptions{})
				gotRule = sanitizeManagedPrometheusRule(gotRule) // Remove variations.
				require.NoError(t, err)
		...
		"Having SLOs with custom time windows (28 day) should generate Managed Prometheus operator CRD.": {
			sloPeriod: "28d",
			exec: func(ctx context.Context, t *testing.T, ns string, kubeClis *k8scontroller.KubeClients) {
				// Prepare our SLO on Kubernetes.
				SLOs := getBaseManagedPrometheusServiceLevel()
				...
				// Check.
				expRule := getBase28DayManagedPromOpRules(version)
				expRule.Namespace = ns
		...
		"Having SLOs with plugins as a CRD should generate Managed Prometheus operator CRD.": {
			sloPeriod: "30d",
			exec: func(ctx context.Context, t *testing.T, ns string, kubeClis *k8scontroller.KubeClients) {
				// Prepare our SLO on Kubernetes with plugin based SLO.
				SLOs := getPluginManagedPrometheusServiceLevel()
				_, err := kubeClis.Sloth.SlothV1().ManagedPrometheusServiceLevels(ns).Create(ctx, SLOs, metav1.CreateOptions{})
				require.NoError(t, err)
				...
				// Check.
				expRule := getPluginManagedPromOpRules(version)
				expRule.Namespace = ns
		...
		"Having SLOs as a CRD should set the status as correct on the CRD.": {
			...
			exec: func(ctx context.Context, t *testing.T, ns string, kubeClis *k8scontroller.KubeClients) {
				// Prepare our SLO on Kubernetes.
				SLOs := getBaseManagedPrometheusServiceLevel()
				newSLOs, err := kubeClis.Sloth.SlothV1().ManagedPrometheusServiceLevels(ns).Create(ctx, SLOs, metav1.CreateOptions{})
				require.NoError(t, err)
				...
				// Check.
				gotSLOs, err := kubeClis.Sloth.SlothV1().ManagedPrometheusServiceLevels(ns).Get(ctx, SLOs.Name, metav1.GetOptions{})
				require.NoError(t, err)

				expStatus := slothv1.ManagedPrometheusServiceLevelStatus{
					ProcessedSLOs:                   2,
					ManagedPromOpRulesGeneratedSLOs: 2,
					ManagedPromOpRulesGenerated:     true,
					ObservedGeneration:              newSLOs.Generation,
				}
				gotSLOs.Status.LastManagedPromOpRulesSuccessfulGenerated = nil // Remove variations.
		...
		"Having wrong SLOs as a CRD should set the status failed on the CRD.": {
			...
			exec: func(ctx context.Context, t *testing.T, ns string, kubeClis *k8scontroller.KubeClients) {
				...
				expStatus := slothv1.ManagedPrometheusServiceLevelStatus{
					ProcessedSLOs:                   2,
					ManagedPromOpRulesGeneratedSLOs: 0,
					ManagedPromOpRulesGenerated:     false,
					ObservedGeneration:              newSLOs.Generation,
				}
				gotSLOs.Status.LastManagedPromOpRulesSuccessfulGenerated = nil // Remove variations.
		...
		"Having SLOs with custom external time windows (7 day) should generate Managed Prometheus operator CRD.": {
			customSLOPeriods: true,
			sloPeriod:        "7d",
			exec: func(ctx context.Context, t *testing.T, ns string, kubeClis *k8scontroller.KubeClients) {
				// Prepare our SLO on Kubernetes.
				...
				// Check.
				expRule := getBase7DayManagedPromOpRules(version)
				expRule.Namespace = ns
